...
createCover = async (dir, files) => {
  let folder;
  let firstFile = files.filter(
    a =>
      a.extension &&
      ["mp4", "mkv", "avi", "ogg", "webm", "rar", "zip"].includes(a.extension.toLocaleLowerCase())
  )[0];
  ...
  if (firstFile) {
    ...
    let FolderCover = path.join(coverPath, folder.Name + ".jpg");

    if (!fs.existsSync(FolderCover)) {
      let img = files.find(
        a => a.extension && ["jpg", "jpeg", "png", "gif"].includes(a.extension.toLocaleLowerCase())
      );
...
PopulateDB = async (folder, files, fd) => {
  let filteredFile = files.filter(f => {
    return (
      f.isDirectory ||
      (["mp4", "mkv", "avi", "ogg", "rar", "zip"].includes(f.extension.toLocaleLowerCase()) &&
        !f.isHidden)
    );
  });
...
scanOneDir = async data => {
  ...
  let folder;
  if (fis.length > 0) folder = await createCover(data.dir, fis);
