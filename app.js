...
app.locals.roles = { User: "User", Manager: "Manager", Administrator: "Administrator" };
...
app.use(function(req, res, next) {
  ...
  app.locals.isAndroid = /(Android)|(iphone)/gi.test(req.get("User-Agent"));
  console.log(req.url);
  if (!req.user && req.url !== "/login") {
    ...
    if (req.url.includes("/admin") && !["Manager", "Administrator"].includes(req.user.Role)) {
      return res.redirect("/");
    }
  }
