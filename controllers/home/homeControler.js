...
var processIndex = async (req, res) => {
  ...
  let isFile = /video|content/gi.test(req.url);
  ...
  let itemsPerPage =
    parseInt(req.params.items || req.query.items) || req.itemsPerPage;
  //parameters
  ...
  let query = {
    ...
    limit: itemsPerPage,
    attributes: {
      include: [[db.sqlze.fn("REPLACE", db.sqlze.col("Name"), "[", "0"), "N"]]
    },
    where: {
      [db.Op.or]: searchs
    }
...
exports.postSearch = (req, res) => {
  ...
  if (/video|content|manga/gi.test(req.url)) {
    url =
      (folderId
        ? `/folder-content/${orderby}/${folderId}`
        : /mangas/gi.test(req.url)
        ? `/mangas/${orderby}`
